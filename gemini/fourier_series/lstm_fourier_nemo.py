...
def preprocess_data(df, seq_length=5, window_size=3, frequencies=[1/24, 1/7, 1/30]):
    ...
    fourier_embeddings_array = np.array([fourier_embeddings(x, frequencies) for x in df['time_index']])
    df['fourier_embeddings'] = [x.tolist() for x in fourier_embeddings_array]

    # Define feature and target columns
    features = df[['price', 'amount', 'price_ma', 'price_volatility', 'price_momentum', 'rsi']].values
    Fourier_features = np.array([np.array(x) for x in df['fourier_embeddings'].tolist()])

    # Concatenate features
    concatenated_features = np.concatenate((features, Fourier_features), axis=1)
    ...
    X, y = create_sequences(concatenated_features, seq_length)
