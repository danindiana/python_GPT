...
from tensorflow.keras.layers import Input, LSTM, Dense, GRU, Dropout, BatchNormalization
...
def preprocess_data(df, seq_length=5, window_size=3):
    ...
    features = df[['price', 'amount', 'price_ma', 'price_volatility']].values
...
def build_model(input_shape):
    model = Sequential([
        ...
        LSTM(150, return_sequences=True, dropout=0.1, recurrent_dropout=0.1, kernel_regularizer=tf.keras.regularizers.l2(0.01)),
        LSTM(150, return_sequences=False, dropout=0.1, recurrent_dropout=0.1, kernel_regularizer=tf.keras.regularizers.l2(0.01)),
        Dense(100, kernel_regularizer=tf.keras.regularizers.l2(0.01)),
...
def train_model(model, train_dataset, val_dataset, epochs=30, steps_per_epoch=None):
