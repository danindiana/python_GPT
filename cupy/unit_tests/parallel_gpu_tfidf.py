...
def save_results_to_file(file_names, tf_idf_matrix, terms, output_file):
...
def process_text_files(directory, chunk_size, save_to_disk=False):
    ...
    while start_index < total_files:
        ...
        if save_to_disk:
            output_file = f"tfidf_results_chunk_{chunk_num}.csv"
            save_results_to_file(file_names, combined_tf_idf, terms, output_file)
