...
def prompt_for_download_pdfs():
    """Prompts the user to download discovered PDF files during the crawl."""
    response = input("Do you want to download discovered PDF files during the crawl? (y/n) [default: n]: ").strip().lower()
    return response == "y"

def prompt_for_worker_count():
    """Prompts the user for the number of workers to spawn, defaulting to 10."""
    worker_count = input("Enter the number of workers to spawn [default: 10]: ").strip()
    return int(worker_count) if worker_count.isdigit() else 10
...
def download_file(session, url, download_directory, max_attempts=3):
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            time.sleep(random.uniform(1, 23))  # Random delay between 1 and 3 seconds
...
def crawl_and_download(session, start_url, download_directory, file_type, download_pdfs, max_depth=3):
    ...
    while urls_to_crawl and not shutdown_flag:
        ...
        try:
            ...
            if e.response.status_code == 401:
                print(f"Error crawling {current_url}: 401 Unauthorized. Skipping.")
            elif e.response.status_code == 404:
                print(f"Error crawling {current_url}: 404 Not Found. Skipping.")
            else:
                print(f"Error crawling {current_url}: {e}. Skipping.")
            continue
        except requests.exceptions.ConnectionError as e:
            # Handle connection errors (e.g., NameResolutionError, Max Retries Exceeded)
            if "NameResolutionError" in str(e):
                print(f"Error crawling {current_url}: Failed to resolve domain name. Skipping.")
            elif "Max retries exceeded" in str(e):
                print(f"Error crawling {current_url}: Max retries exceeded. Skipping.")
            else:
                print(f"Error crawling {current_url}: Connection error. Skipping.")
            continue
        except requests.exceptions.Timeout as e:
            # Handle timeout errors
            print(f"Error crawling {current_url}: Timeout. Skipping.")
            continue
        except requests.exceptions.RequestException as e:
            ...
            print(f"Unexpected error crawling {current_url}: {e}. Skipping.")
            continue
        ...
        if file_type in response.headers.get("Content-Type", "") or current_url.endswith(f".{file_type}"):
            download_file(session, current_url, download_directory)
        # If downloading PDFs is enabled and the file is a PDF, download it
        elif download_pdfs and ("application/pdf" in response.headers.get("Content-Type", "") or current_url.endswith(".pdf")):
            download_file(session, current_url, download_directory)

def download_links_from_json(json_file, file_type, download_pdfs, max_workers=10, crawl_depth=3):
            ...
            for item in data:
                if 'urls' in item and isinstance(item['urls'], list):
                    for url in item['urls']:
                        ...
                        futures.append(executor.submit(crawl_and_download, session, url, download_directory, file_type, download_pdfs, crawl_depth))
...
if __name__ == "__main__":
    ...
    # Prompt to download discovered PDF files
    download_pdfs = prompt_for_download_pdfs()

    # Prompt for the number of workers to spawn, defaulting to 10
    max_workers = prompt_for_worker_count()

    # Start the download process
    download_links_from_json(json_file, file_type, download_pdfs, max_workers, crawl_depth=3)