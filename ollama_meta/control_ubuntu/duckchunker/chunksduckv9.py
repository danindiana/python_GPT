...
def get_running_model_and_port():
    ...
    if len(lines) > 1:
        ...
        if len(model_info) > 0:
            ...
            # Assuming the port is the last element in the line
            port = model_info[-1]
...
def process_chunk(chunk_text, model, port):
    try:
        # Run ollama with the specified model and port, and pass the chunk as input
        process = run(["ollama", "run", model, "--port", port], input=chunk_text, text=True, stdout=PIPE, stderr=PIPE, check=True)
    ...
    for file_name in os.listdir(source_dir):
        if file_name.endswith(".txt"):
            ...
            for i, chunk in enumerate(chunks):
                ...
                result = process_chunk(chunk, model, port)
