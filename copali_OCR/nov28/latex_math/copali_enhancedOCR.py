...
def preprocess_image_for_ocr(image):
    """Preprocess the image for better OCR accuracy, especially for math symbols."""
    image = image.convert("L")  # Convert to grayscale
    image = ImageOps.autocontrast(image)  # Improve contrast
    image = ImageOps.expand(image, border=10, fill='white')  # Add padding
    image = image.resize((image.width * 2, image.height * 2), Image.LANCZOS)  # Upscale for details
    return image
...
def combine_results(tesseract_text, direct_text):
    """Combine results from OCR and direct text extraction."""
    combined_text = "\n".join(filter(None, [direct_text, tesseract_text]))
...
def extract_images_and_text_ocr(pdf_path, resize_factor=2):
    ...
    for page_number, page in enumerate(pdf):
        ...
        try:
            tesseract_text = pytesseract.image_to_string(processed_image, config=custom_tesseract_config)
        except pytesseract.TesseractError as e:
            print(f"Tesseract OCR failed for page {page_number} in {pdf_file}: {e}")
            tesseract_text = ""

        combined_text = combine_results(tesseract_text, pdf_text)
        ocr_text += f"\n--- Page {page_number + 1} ---\n" + combined_text
