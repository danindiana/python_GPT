To modify the `copali_pdf_UI.py` script to recursively find PDF files in the source directory (`/mnt/Incoming_Pdfs`), you can use the `os.walk` function to traverse the directory tree. Here is the updated script with the recursive find feature:

### Modified `copali_pdf_UI.py` Script:
...
for pdf_file in pdf_files:
    ...
    output_file = os.path.join(output_dir, f"{os.path.basename(pdf_file)}_ocr_output.txt")
...
1. **Added `find_pdf_files` Function:**
   - This function uses `os.walk` to recursively find all PDF files in the given directory.

2. **Modified PDF File Listing:**
   - Replaced the static listing of PDF files with a call to `find_pdf_files(input_dir)` to get a list of all PDF files recursively.

3. **Updated Output File Naming:**
   - Used `os.path.basename(pdf_file)` to ensure the output file name is based on the original PDF file name.

By following these steps, the script will now recursively find all PDF files in the specified directory and process them accordingly.
