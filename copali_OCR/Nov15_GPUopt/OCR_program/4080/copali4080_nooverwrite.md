To ensure that the program does not overwrite existing output files, you can add a check to see if the output file already exists in the target directory. If it does, you can either skip processing that file or append a suffix to the output file name to make it unique.
...
1. **Added `get_unique_output_file` Function:**
   - This function generates a unique output file name by appending a counter to the base file name if the file already exists in the output directory.

2. **Modified Output File Naming:**
   - Used the `get_unique_output_file` function to ensure that the output file name is unique and does not overwrite existing files.

By following these steps, the script will now recursively find all PDF files in the specified directory, process them, and ensure that the output files do not overwrite existing files in the target directory.
