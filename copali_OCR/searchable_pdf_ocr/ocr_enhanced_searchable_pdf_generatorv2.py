    ...
    for page_num, page in enumerate(doc, start=1):
        ...
        for i, word in enumerate(ocr_data['text']):
            if word.strip():  # Ignore empty text
                ...
                page.insert_textbox(rect, word, fontsize=10, color=(1, 1, 1, 0), overlay=True)
...
def extract_images_and_create_searchable_pdf(pdf_path, output_pdf, resize_factor=2):
    ...
    for page_number, page in enumerate(pdf):
        ...
        # Adjust bounding box coordinates to match the original PDF dimensions
        scale_x = width / pil_image.width
        scale_y = height / pil_image.height
        for i in range(len(page_ocr_data['text'])):
            page_ocr_data['left'][i] = int(page_ocr_data['left'][i] * scale_x)
            page_ocr_data['top'][i] = int(page_ocr_data['top'][i] * scale_y)
            page_ocr_data['width'][i] = int(page_ocr_data['width'][i] * scale_x)
            page_ocr_data['height'][i] = int(page_ocr_data['height'][i] * scale_y)
