...
def deduplicate_text(texts):
    """Remove near-duplicates based on hash or cosine similarity."""
    unique_texts = []
    seen_hashes = set()
...
for pdf_file in pdf_files:
    ...
    if pdf_text.strip():
        ...
        # Detect language and filter out non-English documents
        lang, confidence = detect_language(pdf_text)
        if lang != '__label__en':
            print(f"Skipping file {pdf_file} as it is not in English.")
            skipped_files.append(pdf_file)
            continue

        # Deduplicate text
        unique_texts = deduplicate_text([pdf_text])
        if not unique_texts:
            print(f"Skipping file {pdf_file} as it is a duplicate.")
            skipped_files.append(pdf_file)
            continue

        # Dynamically split text into manageable chunks based on max_chunk_size
        text_chunks = split_text_into_chunks(unique_texts[0], max_chunk_size)
        similarity_scores = []
